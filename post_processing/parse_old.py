...
def split_file(src_file, key_word, file_encode='utf-8'):
    """

    :param src_file: specified the source file
    :param key_word: specified the keyword
    :param file_encode:
    :return:
    """
...
def gen_pg_sql_single(src_file, key_word, index_first, file_code='utf-8'):
    """
    post_processing the file
    """
    regex = re.compile(key_word)
    ...
    with codecs.open(src_file, 'r', encoding='utf-8') as fr, \
        ...
        while True:
            ...
            if line.count(key_word) == 1:
                ...
                latest_line =  'INSERT INTO ' + key_word + ' VALUES (' + deal_with_col_single(replace_line, ',', index_first).replace("\'array", 'array').replace(']\'', ']') + ');' + '\n'
...
def deal_with_col_single(line: str, separator=',', index_first=2):
    col_array = line.split(separator)
    for index, elem in enumerate(col_array):
        # if elem.find('|') > 0:
...
if __name__ == '__main__':
    ...
    for item in ['cc_crm_member', 'wechatfans', 'wechatMinApp',
                 'Shopper', 'register_member', 'add_to_cart', 'buy_event']:
        split_file('test20230811.csv', item)
        # parse_file('test20230811.csv', item)

    # # for item in ['wechatfans']:
    # #     gen_pg_sql('mock_huaxi.csv', item, 4, 5)
    # for item in ['Shopper']:
    #     gen_pg_sql_single('mock_lastest2023.csv', item, 2)


    # for item in ['cc_crm_member', 'wechatfans', 'wechatMinApp', 'Shopper']:
    #     parse_file('mock_lastest2023.csv', item)
